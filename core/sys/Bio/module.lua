...
local function normalize_impl(unit, outer)
	...
	if unit.id_hash == M.chemical_id_hash and unit.thing then
		M.normalize_chemical(unit, outer)
	elseif #unit.items > 0 then
		M.normalize_items(unit, outer)
	end
end

function M.normalize_chemical(unit, outer)
	local common_unit = outer and outer:unit() or munit_milligram
...
function M.normalize_items(unit, outer)
	...
	elseif outer.value ~= 0 and outer.value ~= inner_sum.value then
		...
		normalize_impl(imbalance_unit, outer)
